        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function formatJamTanggal(pakaiDetik) {
                ...
                const jam = now.toLocaleTimeString('id-ID', {
                    ...
                    hour: '2-digit',
                    minute: '2-digit',
                    second: pakaiDetik ? '2-digit' : undefined
                });
            ...
            function updateClock() {
                ...
                const watermarkWaktu = document.getElementById('watermarkWaktu');

                if (watermarkWaktu && modalKamera && !modalKamera.classList.contains('hidden')) {
                    watermarkWaktu.textContent = tanggal + ' \u2022 ' + jam;
                }
            }
            ...
            function updatePratinjauWatermark() {
                ...
                if (!data || data.jarak === null) {
                    watermarkJarakEl.textContent = 'Lokasi belum diambil';
                    return;
                }

                const dalamRadius = data.jarak <= OFFICE_LOKASI.radius;

                watermarkJarakEl.textContent =
                    '\uD83D\uDCCD ' + Math.round(data.jarak) + ' m dari kantor \u00b7 ' +
            ...
            if (btnTutupKamera) {
                btnTutupKamera.addEventListener('click', tutupKamera);
            }
            ...
            function gambarWatermark(context, width, height, jenis) {
                ...
                const lokasiText = (data.lat !== null && data.lng !== null)
                ...
                gradient.addColorStop(1, 'rgba(10, 10, 20, 0.80)');
                ...
                context.fillText(tanggal + ' \u2022 ' + jam, paddingX, y);
            ...
            if (btnAmbilKamera) {
                btnAmbilKamera.addEventListener('click', function () {
                    ...
                    const width = videoKamera.videoWidth;
                    const height = videoKamera.videoHeight;

                    canvasKamera.width = width;
                    canvasKamera.height = height;

                    const context = canvasKamera.getContext('2d');

                    context.drawImage(videoKamera, 0, 0, width, height);

                    gambarWatermark(context, width, height, jenisAktif);
                    ...
                    canvasKamera.toBlob(function (blob) {
                        ...
                        if (keterangan) {
                            document.getElementById(keterangan).textContent =
                                'Foto berhasil diambil (sudah dilengkapi watermark waktu & lokasi).';
                        }
            ...
            if (btnFotoMasuk) {
                btnFotoMasuk.addEventListener('click', () => bukaKamera(fotoMasuk, 'masuk'));
            }

            if (btnFotoUlangMasuk) {
                btnFotoUlangMasuk.addEventListener('click', () => bukaKamera(fotoMasuk, 'masuk'));
            }

            if (btnFotoPulang) {
                btnFotoPulang.addEventListener('click', () => bukaKamera(fotoPulang, 'pulang'));
            }

            if (btnFotoUlangPulang) {
                btnFotoUlangPulang.addEventListener('click', () => bukaKamera(fotoPulang, 'pulang'));
            }
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    navigator.geolocation.getCurrentPosition(
                            ...
                            statusElement.innerHTML =
                                ...
                                '<span>' +
                                    ...
                                    '<span class="text-xs ' + (dalamRadius ? 'text-emerald-500' : 'text-red-500') + '">' +
                                        (dalamRadius
                                            ? 'Dalam radius kantor (maksimal ' + OFFICE_LOKASI.radius + ' m)'
                        ...
                        function (error) {
                            ...
                            if (error.code === 1) {
                                alert('Akses lokasi ditolak. Silakan izinkan lokasi pada browser.');
                            } else if (error.code === 2) {
                                alert('Lokasi tidak dapat ditemukan. Pastikan GPS/lokasi perangkat aktif.');
                            } else if (error.code === 3) {
                                alert('Waktu pengambilan lokasi habis. Silakan coba lagi.');
                            } else {
            ...
            function cekFormMasuk() {
                ...
                button.disabled = !(shift && latitude && longitude && foto);
            }
            ...
            function cekFormPulang() {
                ...
                button.disabled = !(latitude && longitude && foto);
            }
